#' The orness degree of a Winsorized weighted mean
#'
...
#'
#' @return A numercial value. The orness degree of the Winsorized weighted mean required
#' @export
#'
#' @examples
#' orness_winsorized_weighted_mean(weights = rep(0.2,10), r=2, s=2)
orness_winsorized_weighted_mean<-function(weights, r, s=r, alpha=NULL, beta=alpha){
  ...
  orness <- 1 / 2 + (r*(r + 1) - s*(s + 1)) / (2*n*(n - 1))

  return(orness)
}
# test con ornes con r y s iguales debe dar 0.5
